    ...
    <div className="space-y-5">
      ...
      <Reveal direction="up">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="font-display text-[26px] sm:text-[30px] text-foreground">{L("שירותים", "الخدمات", "Services")}</h1>
            <p className="text-[13px] text-muted-foreground mt-0.5">{L(`${services.length} שירותים`, `${services.length} خدمات`, `${services.length} services`)}</p>
          </div>
          <button
          ...
          >
            <Plus className="h-4 w-4" />{L("שירות חדש", "خدمة جديدة", "New Service")}
          </button>
        </div>
      </Reveal>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-surface/60 border-b border-border/15">
                  ...
                  <th className="text-start p-3.5 text-[11px] font-bold uppercase tracking-[0.08em] text-muted-foreground hidden sm:table-cell">{L("משך", "المدة", "Duration")}</th>
                  ...
                  <tr key={s.id} className="border-t border-border/10 hover:bg-surface/30 transition-colors">
                    <td className="p-3.5">
                      <div className="flex items-center gap-3">
                        {s.image_url ? (
                          <img src={s.image_url} alt="" className="h-9 w-9 rounded-lg object-cover shrink-0" />
                        ) : (
                        ...
                        <span className="font-medium text-foreground">{lang === "ar" ? s.name_ar || s.name : s.name}</span>
                      </div>
                    </td>
                    <td className="p-3.5 text-muted-foreground hidden sm:table-cell">
                      {s.category && (
                        <span className="text-[11px] font-medium bg-surface px-2.5 py-1 rounded-lg">{s.category}</span>
                      )}
                    </td>
                    ...
                    <td className="p-3.5 hidden sm:table-cell">
                      <span className="inline-flex items-center gap-1.5 text-[12px] text-muted-foreground">
                        ...
                        {s.duration_minutes}{L("ד'", "د", "m")}
                      </span>
                    </td>
                    <td className="p-3.5 hidden sm:table-cell">
                      <span className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full border ${s.is_active ? "bg-sage-soft text-sage border-sage/20" : "bg-surface text-muted-foreground border-border/30"}`}>
                        <span className={`h-1.5 w-1.5 rounded-full ${s.is_active ? "bg-sage" : "bg-muted-foreground/50"}`} />
                        {s.is_active ? L("פעיל", "نشط", "Active") : L("מושבת", "غير نشط", "Inactive")}
                      </span>
                    </td>
                    <td className="p-3.5 text-end">
                      <div className="inline-flex gap-0.5">
                        <Button size="icon" variant="ghost" onClick={() => toggle(s)} className="h-8 w-8 rounded-lg hover:bg-surface">
                          {s.is_active ? <EyeOff className="h-3.5 w-3.5 text-muted-foreground" /> : <Eye className="h-3.5 w-3.5 text-muted-foreground" />}
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => setDlg({ open: true, row: s })} className="h-8 w-8 rounded-lg hover:bg-surface">
                          <Pencil className="h-3.5 w-3.5 text-muted-foreground" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => del(s.id)} className="h-8 w-8 rounded-lg hover:bg-destructive/10">
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={6} className="py-16 text-center">
                      ...
                      <div className="text-[14px] font-medium text-muted-foreground">{search ? L("לא נמצאו תוצאות", "لم يتم العثور على نتائج", "No results found") : L("אין שירותים עדיין", "لا خدمات بعد", "No services yet")}</div>
                      <div className="text-[12px] text-muted-foreground/60 mt-1">{search ? L("נסה חיפוש אחר", "جرب بحثاً آخر", "Try a different search") : L("הוסף שירות ראשון", "أضف الخدمة الأولى", "Add your first service")}</div>
                    </td>
                  </tr>
                )}
              </tbody>
