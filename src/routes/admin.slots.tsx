...
function Page() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="font-display text-2xl">{L("חלונות זמן", "أوقات الحجز", "Appointment Slots")}</h1>

      <div className="rounded-2xl border border-border/60 bg-card p-4 soft-shadow">
        <div className="grid sm:grid-cols-5 gap-3 items-end">
          <div className="grid gap-1.5 sm:col-span-2">
            <Label className="text-xs">{L("שירות", "الخدمة", "Service")}</Label>
          ...
          <div className="grid gap-1.5">
            <Label className="text-xs">{L("תאריך", "التاريخ", "Date")}</Label>
            <Input type="date" value={form.slot_date} onChange={(e) => setForm({ ...form, slot_date: e.target.value })} className="h-10" />
          </div>
          <div className="grid gap-1.5">
            <Label className="text-xs">{L("התחלה", "البداية", "Start")}</Label>
            <Input type="time" value={form.start_time} onChange={(e) => setForm({ ...form, start_time: e.target.value })} className="h-10" />
          </div>
          <div className="grid gap-1.5">
            <Label className="text-xs">{L("סיום", "النهاية", "End")}</Label>
            <Input type="time" value={form.end_time} onChange={(e) => setForm({ ...form, end_time: e.target.value })} className="h-10" />
          </div>
          <Button className="btn-gold h-10 sm:col-span-5 sm:w-fit" onClick={add}><Plus className="h-4 w-4 me-1" />{L("הוסף", "أضف", "Add")}</Button>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-surface text-secondary-foreground">
              <tr>
                ...
                <th className="text-start p-3">{L("שעות", "الوقت", "Time")}</th>
                <th className="text-start p-3">{L("זמין", "متاح", "Available")}</th>
            ...
            <tbody>
              {slots.map((s: any) => (
                <tr key={s.id} className="border-t border-border/40">
                  ...
                  <td className="p-3 text-end"><Button size="icon" variant="ghost" onClick={() => del(s.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></td>
                </tr>
              ))}
              {slots.length === 0 && <tr><td colSpan={5} className="p-6 text-center text-secondary-foreground">—</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
