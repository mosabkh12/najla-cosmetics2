  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-2xl">{L("מוצרים", "المنتجات", "Products")}</h1>
        <Button className="btn-gold" onClick={() => setDlg({ open: true, row: { is_active: true, stock_quantity: 0, low_stock_threshold: 5 } })}>
          ...
          <table className="w-full text-sm">
            <thead className="bg-surface text-secondary-foreground">
              <tr>
                ...
                <th className="text-start p-3">{L("מלאי", "المخزون", "Stock")}</th>
              ...
              {products.map((p: any) => {
                ...
                return (
                  <tr key={p.id} className="border-t border-border/40">
                    ...
                    <td className="p-3 text-secondary-foreground">{p.category}</td>
                    ...
                    <td className="p-3 text-end">
                      <div className="inline-flex gap-1">
                        <Button size="icon" variant="ghost" onClick={() => toggle(p)}>{p.is_active ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</Button>
                        <Button size="icon" variant="ghost" onClick={() => setDlg({ open: true, row: p })}><Pencil className="h-4 w-4" /></Button>
