...
function ProfilePage() {
  ...
  const [apptFilter, setApptFilter] = useState<"all" | "upcoming" | "completed" | "cancelled">(
    "all",
  );
  const [orderFilter, setOrderFilter] = useState<"all" | "active" | "completed" | "cancelled">(
    "all",
  );
          ...
          <div className="mt-6 flex gap-2 overflow-x-auto pb-1">
            {sectionItems.map((item) => (
              ...
              >
                ...
                {(item.count ?? 0) > 0 && (
                  <span
                    className={`grid h-5 min-w-5 place-items-center rounded-full text-[10px] font-bold ${
                      section === item.key
            ...
            <Reveal direction="up" className="mb-5">
              ...
              {hasApptHistory && (
                <p className="mt-2 text-[11px] text-muted-foreground">
                  {t("appointment_auto_delete_notice")}
                </p>
              )}
            </Reveal>
            ...
            <StaggerGrid className="space-y-3">
              {filteredAppts.map((a) => {
                ...
                return (
                  <div
                    key={a.id}
                    className="rounded-2xl bg-card p-4 transition-all hover:shadow-md"
            ...
            <StaggerGrid className="grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
              {favs.map((p) => (
                <Link
                  ...
                  to="/products/$id"
                  params={{ id: p.id }}
                  className="group block rounded-2xl bg-card p-2.5 transition-all hover:shadow-md"
                  style={{ boxShadow: "0 10px 30px -10px rgba(45, 45, 45, 0.04)" }}
                >
