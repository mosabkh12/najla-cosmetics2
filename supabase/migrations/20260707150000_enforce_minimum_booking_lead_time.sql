-- getAvailableTimes (src/api/appointments/appointments.ts) already hides
-- any slot less than 30 minutes from now as a UX courtesy, but that was
-- UI-only — these RPCs, the actual authority, only ever rejected times
-- strictly in the past. Nothing stopped a booking 1-2 minutes from now if
-- made some other way than the normal slot list. Both functions below are
-- otherwise byte-for-byte identical to 20260706230500's versions — only
-- the PAST_TIME check changes, from "before now" to "before now + 30min".
