-- =============================================
-- Fix: deleting a service that has ever been booked was silently blocked
--
-- Same root cause as 20260715120000 for products/order_items:
-- appointments.service_id was NOT NULL REFERENCES services(id) with no
-- ON DELETE action (defaults to blocking the delete). Any service that
-- had ever been booked into a single appointment could therefore never
-- be deleted.
--
-- Unlike order_items (which already snapshotted product_name/unit_price
-- at order time), appointments had NO service-name snapshot at all —
-- admin/customer appointment lists render via a LIVE join to services
-- (getAdminAppointments: "service:services(name,name_ar)",
-- getUserAppointments: "services(name,name_ar,image_url)"). Simply
-- setting service_id to null on delete would blank out the service name
-- on every past appointment for that service. So this migration adds a
-- name snapshot first (service_name/service_name_ar, matching exactly
-- the two columns those two live joins already select), backfills it
-- for existing rows, THEN relaxes service_id to nullable + ON DELETE
-- SET NULL. The application code falls back to this snapshot whenever
-- the live join comes back null (service since deleted).
-- =============================================

ALTER TABLE public.appointments
  ADD COLUMN service_name TEXT,
  ADD COLUMN service_name_ar TEXT;
...
-- ═══════════════════════════════════════════════
-- create_appointment / reschedule_appointment — snapshot the service
-- name at the moment it's booked/changed, same signatures (CREATE OR
-- REPLACE, no signature change needed).
-- ═══════════════════════════════════════════════
...
BEGIN
  ...
  INSERT INTO public.appointments (
    ...
    v_service.name, v_service.name_ar
  )
