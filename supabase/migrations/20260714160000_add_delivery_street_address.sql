...
--
-- Delivery orders need more than "which priced area" — the courier
-- needs an actual street to go to. The city/area is still the existing
-- delivery_areas picker (that's what determines the fee); this adds a
-- free-text street address the customer must provide whenever
-- delivery_method = 'delivery', stored alongside the area/fee snapshot
-- already on orders.
-- =============================================
...
ALTER TABLE public.orders
  ADD CONSTRAINT orders_delivery_street_length CHECK (
    delivery_street IS NULL OR length(delivery_street) <= 200
  );

-- Replace the pickup/delivery consistency check to also require (and
-- confine) a non-blank street exactly when delivery_method = 'delivery'.
ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS orders_delivery_area_consistency;
ALTER TABLE public.orders
  ADD CONSTRAINT orders_delivery_area_consistency CHECK (
    (delivery_method = 'pickup' AND delivery_area_id IS NULL AND delivery_fee = 0 AND delivery_street IS NULL)
    OR (delivery_method = 'delivery' AND delivery_area_id IS NOT NULL AND length(btrim(delivery_street)) > 0)
  );


-- ═══════════════════════════════════════════════
-- create_order RPC — extended with p_delivery_street
--    Same drop-and-recreate approach as the previous migration: the
--    7-arg overload is dropped outright rather than left dangling.
-- ═══════════════════════════════════════════════

DROP FUNCTION IF EXISTS public.create_order(UUID, TEXT, TEXT, TEXT, TEXT, UUID, JSONB);
...
REVOKE ALL ON FUNCTION public.create_order(UUID, TEXT, TEXT, TEXT, TEXT, UUID, TEXT, JSONB) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_order(UUID, TEXT, TEXT, TEXT, TEXT, UUID, TEXT, JSONB) TO service_role;
